...
def test_boxplot_stats(chinook_db, custom_ip):
    custom_ip.run_cell("%sql duckdb://")
    custom_ip.run_cell("%sql INSTALL 'sqlite_scanner';")
    custom_ip.run_cell("%sql commit")
    custom_ip.run_cell("%sql LOAD 'sqlite_scanner';")
    custom_ip.run_cell(f"%sql CALL sqlite_attach({chinook_db!r});")

    res = custom_ip.run_cell("%sql SELECT * FROM Invoice").result
...
def test_boxplot_stats_exception(chinook_db, custom_ip):
    custom_ip.run_cell("%sql duckdb://")
    custom_ip.run_cell("%sql INSTALL 'sqlite_scanner';")
    custom_ip.run_cell("%sql commit")
    custom_ip.run_cell("%sql LOAD 'sqlite_scanner';")
...
def test_summary_stats(chinook_db, custom_ip, tmp_empty):
...
def test_summary_stats_missing_file(chinook_db, custom_ip):
    custom_ip.run_cell("%sql duckdb://")
    custom_ip.run_cell("%sql INSTALL 'sqlite_scanner';")
    custom_ip.run_cell("%sql commit")
    custom_ip.run_cell("%sql LOAD 'sqlite_scanner';")
